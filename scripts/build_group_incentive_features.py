#!/usr/bin/env python
"""Build World Cup group incentive snapshots from local fixtures/results.
...
from football_predictor.worldcup.enrichment import build_group_state_snapshots
...
def main() -> None:
    ...
    cutoff = parse_datetime(args.cutoff) if args.cutoff else utc_now()
    if cutoff is None:
        raise SystemExit("--cutoff must be an ISO datetime")
    ...
    with session_scope(session_factory) as session:
        result = build_group_state_snapshots(
            session,
            cutoff=cutoff,
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cutoff", default=None, help="ISO cutoff. Defaults to now.")

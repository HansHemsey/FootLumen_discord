...
def worldcup_build_dataset(
    ...
    if audit_teams:
        ...
        with session_scope(session_factory) as session:
            teams = _worldcup_fixture_teams(session)
    frame, coverage = build_and_save_worldcup_dataset(bundle, output=output, audited_teams=teams)
    console.print(
        {
            "rows": len(frame),
            "columns": len(frame.columns),
...
def worldcup_backtest_1x2(
    ...
    worldcup_test = (
        result.metrics.get("metrics_by_model", {})
        .get("worldcup_1x2", {})
        .get("test", {})
    )
    console.print(
...
# ---------------------------------------------------------------------------
# Over/Under 2.5 sub-application
# ---------------------------------------------------------------------------
...
def ou_build_dataset(
    ...
    output: Path = typer.Option(
        Path("data/processed/training_ou_v1.parquet"), "--output"
    ),
    limit: int | None = typer.Option(None, "--limit"),
) -> None:
